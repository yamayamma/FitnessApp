...
final class HistoryViewModel {
    ...
    var todaysSessions: [WorkoutSession] {
        ...
        return self.workoutSessions.filter { calendar.isDate($0.startDate, inSameDayAs: today) }
    }
    ...
    var totalDuration: TimeInterval {
        self.workoutSessions.reduce(0) { $0 + $1.totalDuration }
    }

    /// 基本統計: 合計ワークアウト数
    var totalWorkouts: Int {
        self.workoutSessions.count
    }
    ...
    func loadSessions(modelContext: ModelContext) {
        ...
        do {
            self.workoutSessions = try modelContext.fetch(descriptor)
        } catch {
