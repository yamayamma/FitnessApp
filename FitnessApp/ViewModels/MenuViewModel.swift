...
final class MenuViewModel {
    ...
    func loadMenus(modelContext: ModelContext) {
        ...
        do {
            menus = try modelContext.fetch(descriptor)
        } catch {
    ...
    func createMenu(name: String, modelContext: ModelContext) {
        ...
        do {
            ...
            loadMenus(modelContext: modelContext)
            syncToWatch()
        } catch {
    ...
    /// 種目を並び替え
    func reorderExercises(in menu: TrainingMenu, from source: IndexSet, to destination: Int, modelContext: ModelContext) {
    ...
    private func syncToWatch() {
        let transfers = menus.map { menu in
            MenuTransfer(
                menuId: menu.menuId,
                name: menu.name,
