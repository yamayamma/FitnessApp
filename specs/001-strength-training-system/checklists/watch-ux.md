...
- [ ] CHK004 — 画面遷移のアニメーション・トランジション要件は定義されているか？（NavigationStack push/pop 等） [Gap]
...
- [ ] CHK011 — 最終セット完了時の「次セット」ボタンの挙動要件は定義されているか？（自動的にワークアウト完了に遷移？次の種目に遷移？） [Gap, Edge Case]

## Requirement Completeness — Workout Controls

- [ ] CHK012 — 一時停止/再開/キャンセル/完了のコントロール UI の配置・操作方法の要件は定義されているか？ [Gap, Spec §FR-001]
- [ ] CHK013 — キャンセル操作の確認ダイアログの要件は定義されているか？（誤タップ防止） [Gap, Edge Case]
- [ ] CHK014 — ワークアウト完了時の確認 UI の要件は定義されているか？ [Gap]
- [ ] CHK015 — 次の種目への遷移操作の要件は定義されているか？（FR-004 はセット進行のみ、種目間遷移が未定義） [Gap, Spec §FR-004]
